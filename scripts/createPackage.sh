...
PACKAGE_VERSION="$(execute sf package version create -p $PACKAGE_NAME -f config/org-package-def.json -x -w 10 --code-coverage --json | jq '.result.SubscriberPackageVersionId' | tr -d '"')"
